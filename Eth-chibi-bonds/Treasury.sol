...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, Schibi }

    IKeplerERC20 immutable chibi;
    ...
    address public schibi;
    uint public schibiQueue; // Delays change to schibi address
    
    uint public totalReserves; // Risk-free value of all assets
    uint public totalDebt;

    // Total 5 arguments. 3rd argument is `address _Frax`, 4th argument is `address _chibiDAI`
    constructor (address _chibi, address _DAI, uint _secondsNeededForQueue) {
        require( _chibi != address(0) );
        chibi = IKeplerERC20(_chibi);

        isReserveToken[ _DAI ] = true;
        reserveTokens.push( _DAI );
        // isReserveToken[ _Frax] = true;
        // reserveTokens.push( _Frax );
        // isLiquidityToken[ _chibiDAI ] = true;
        // liquidityTokens.push( _chibiDAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for chibi
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint chibi needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        chibi.mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn chibi for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOfToken( _token, _amount );
        chibi.burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20Extended( schibi ).balanceOf( msg.sender ); // Can only borrow against schibi held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with chibi
        @param _amount uint
     */
    function repayDebtWithchibi( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );
        chibi.burnFrom( msg.sender, _amount );
        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );
        emit RepayDebt( msg.sender, address(chibi), _amount, _amount );
    }
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( chibi.totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns chibi valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match chibi decimals
            value_ = _amount.mul( 10 ** chibi.decimals() ).div( 10 ** IERC20Extended( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.Schibi ) { // 9
            schibiQueue = block.timestamp.add( secondsNeededForQueue );
        } else return false;
