...
contract BondDepository is Ownable {
    ...
    address public immutable chibi; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints chibi when receives principle
    ...
    struct Bond {
        uint payout; // chibi remaining to be paid
    ...
    constructor ( address _chibi, address _principle, address _treasury, address _DAO, address _bondCalculator) {
    ...
    function deposit( uint _amount, uint _maxPrice, address _depositor) external returns ( uint ) {
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 chibi ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) chibi
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( chibi ).safeTransfer( DAO, fee ); 
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( chibi ).transfer( _recipient, _amount ); // send payout
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( chibi ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to chibi supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( chibi ).totalSupply();
    ...
    /**
     *  @notice calculate amount of chibi available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or chibi) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != chibi );
